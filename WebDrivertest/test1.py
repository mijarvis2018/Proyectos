...
#print 'Locating username input'
#ObjectSelected = driver.find_element_by_xpath("//input[@name='user[email]']")
#ObjectSelected.clear()
#print 'Username located, writing username'
#ObjectSelected.send_keys("usernametest")

 # enter password
#print 'Locating password input'
#ObjectSelected = driver.find_element_by_xpath("//input[@name='user[user_password]']")
#ObjectSelected.clear()
#print 'Password input located, writing password'
#ObjectSelected.send_keys("passwordtest")
#print 'Logging in'
#ObjectSelected.send_keys(Keys.RETURN)
...
try:
    ...
    while len(found_tweets) >= increment:
        ...
        try:
            found_tweets = driver.find_elements_by_css_selector(tweet_selector)
            increment += 20
            print ('Found tweets: %d' % len(found_tweets))
            for tweet in found_tweets:
                try:
                    id = tweet.find_element_by_css_selector(id_selector).get_attribute('href').split('/')[-1]
                    ids.append(id)
